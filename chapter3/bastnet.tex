...
Because translations and candidate-translations need not be surjective on~$V$, we introduce a zero vertex denoted $0_V$, and sometimes called \emph{black hole}~\citep{GrePasViaGri201610}, such that any candidate translation $\phi: U \to V$ is extended as a function $\phi: V \to V \cup \{\bot\}$
\begin{gather}
\forall v \notin U, \phi(v) = 0_V
\end{gather}
