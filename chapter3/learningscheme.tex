...
In our experiments, we learn $S$ and $\Theta$ simultaneously.
...
For the case of citation networks, we consider only one graph and thus we set $\omega = 1$.
...
The GCTs are able to outperform the corresponding CNNs by a small by statistically significant amount in a shallow architecture. Learning the scheme $S$ imply a memory overhead due to the increase in the number of weights of each layer, thus why we limited this experiments to shallow architectures. An example of strategy to extend this experiment to deeper architectures is to tie the schemes of each layer together, or to reuse a same scheme (previously learned) for all layers.
...
\todo{describe experiments and our model}
  ...
  \begin{itemize}
   ...
   \item Addition of graph dropout to GCN (GCN*, this work),
  ...
  \begin{center}
    ...
    \begin{tabular}{|c|c|c|c|c|}
      \hline
      Dataset & GCN & GAT & GCN* & GCT\\
      \hline
      Cora & $81.8 \pm 0.9$ & $83.2 \pm 0.7$ & $\mathbf{83.4} \pm 0.7$ & $83.3 \pm 0.7$\\
      \hline
      Citeseer & $todo \pm todo$ & $72.1 \pm 0.6$ & $72.5 \pm 0.8$ & $\mathbf{72.8} \pm 0.5$\\
      \hline
      Pubmed & $todo \pm todo$ & $78.0 \pm 0.6$ & $todo \pm todo$ & $todo \pm todo$\\
      \hline
    \end{tabular}
    \egroup
  \end{center}
