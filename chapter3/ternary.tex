...
\begin{itemize}
\item a fully connected layer is a neural contraction such that $\omega = nm$ and the slices $S[:,i,j]$ constitute every possible one-hot vectors,
\item a convolution \todo{reword}
...
\item A topology-adaptative graph convolution layer (\cite{du2017topology}, see description in \secref{sec:vert}) is a neural contraction layer for which $S$ contains the powers of $\widetilde{A}$ along the first rank.
...
We replicated the experiment done by \cite{defferrard2016convolutional} on the 20NEWS dataset, which we already discussed about in \secref{sec:spec}.

\todo{explain why not supervised irregular or only a bit irregular, send to bastnet exps}

\todo{Describe our model}

After 100 runs of 10 epochs, we obtained a mean accuracy of $70.74 \pm 2.19\%$, which is a gain that is statistically significant compared to ChebNet ($65.76\%$, see \tabref{tab:20}), but not against our simple MLP ($71.46 \pm 0.08\%$). It is to be noted that the variance of MCNet is quite high and cherry picking the max instead of the mean improve the result to $72.62\%$, which is \todo{}.

\paragraph{Semi-supervised classification of nodes}

\todo{introduce Graph dropout here, and send to semisup exps}
