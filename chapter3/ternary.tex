...
We replicated the experiment done by \cite{defferrard2016convolutional} on the 20NEWS dataset, wchich we already discussed about in \secref{sec:spec}.

\todo{Describe our model}

After 10 runs of 10 epochs, we obtained an accuracy of $71.62 \pm 0.68\%$ which is a gain that is statistically significant compared to ChebNet ($65.76\%$, see \tabref{tab:20}).

\todo{introduce Graph dropout here and refer to later, explain why not supervised irregular and send to semisup or only a bit irregular}
