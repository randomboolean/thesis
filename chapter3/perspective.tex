...
In particular, given an output neuron $u \in U$, a neighborhood $\cn_u$ is also called a \emph{local receptive field} (LRF), that we denote $\ccr_u$.
...
For our definition of edge-constrained layer, we want that the LRF are constrained by the edges of the underlying graph. So we define it as as follows.
...
%In other words, if we identify $U$ with $V_\pi$, an EC layer is a layer such that the propagation graph $P$ resulting from this identification is isomorph to the subgraph of $G$ induced by $U$.
