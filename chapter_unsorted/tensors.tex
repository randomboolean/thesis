...
\begin{remark}
This definition is indeed equivalent with the definition of the tensor product given in~(\cite{hackbusch2012tensor}, p. 51). The drawback of our definition is that it depends on the canonical bases, which is not a concern in our domain as we need not distinguish tensors from their representation.
\end{remark}
...
Let a real-valued function $f$ defined over any tensor space of any shape, \eg the \emph{max} or \emph{average} function.
