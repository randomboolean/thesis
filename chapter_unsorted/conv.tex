...
On graphs, we notice that the property $g(\varphi(h)) = \varphi(gh)$ can be realized by transformations acting on edges. However, unless the graph is complete, these actions can't be replicated everywhere. The algebraic structure that posesses the same kind of properties than a group except that its composition law is not defined everywhere is called a groupoid. The following definitions clarify our discussion.
...
\item a closed partial composition law $gh$ defined if and only if $\psi(g) = \varphi(h)$, which concatenates $g$ behind $h$ and removes adjacent duplicate vertices,
