...
A classic result from group theory is that this characterization hold for groups of (bijective) transformations, where the group of translations is a particular case. More generally, it also holds for groupoids \citep{weinstein1996groupoids}.

\begin{definition}\textbf{Group convolution on $\cs(\group)$}\\
...
\begin{definition}\textbf{Group convolution on $\cs(V \cong \Gamma)$}\\
...
\begin{align*}
(s_1 \ast s_2) [u] & = \displaystyle \sum_{v \in V} s_1[v] \h{2} s_2[\varphi(g_v^{-1}g_u)]\\
& = \displaystyle \sum_{\substack{(a,b) \in V^2 \\ \st g_ag_b=g_u }} s_1[a] \h{2} s_2[b]
\end{align*}
\end{definition}

\begin{proposition}\textbf{Characterization on $\cs(V \cong \Gamma)$}\\
On real-valued signals over $V$, the class of linear transformations that are equivariant to $\varphi(\Gamma)$ is exactly the class of convolutive operations \ie
\begin{gather*}
\begin{cases}
 f \in \cl(\cs(V))\\
 \forall v \in V, f \circ \varphi(g_v) = \varphi(g_v) \circ f
\end{cases}
 \Leftrightarrow \exists w \in \cs(V), f = . \ast w
\end{gather*}
\label{prop:equi}
\end{proposition}

\begin{proof}
i) From right to left (the demonstration of the linearity is omited).
\begin{align*}
...
Let $f \in \cl(\cs(V))$, $s \in \cs(V)$. We suppose that $f$ is equivariant to $\varphi(\Gamma)$.
Similarly as for the proof of \propref{prop:equi}, we have:

\begin{align*}
f(s) & = \displaystyle \sum_{v \in V} s[v] \h{2} f(\delta_v)\\
 & = \displaystyle \sum_{v \in V} s[v] \h{2} f(\delta_{\varphi(g_v)})\\
 & = \displaystyle \sum_{v \in V} s[v] \h{2} f(\varphi(\delta_{g_v}))\\
 %& = \displaystyle \sum_{v \in V} s[v] \h{2} f \circ g_{v} (\delta_{v_0})\\
 %& = \displaystyle \sum_{v \in V} s[v] \h{2} g_{v} \circ f (\delta_{v_0})
\end{align*}
% By denoting $w = f (\delta_{v_0}) \in \cs(V)$, we obtain:
% \begin{align}
% \forall v \in V, f(s)[v] & = \displaystyle \sum_{u \in V} s[u] \h{2} g_{u}(w)[v] \label{eq:conv}\\
% % & = \displaystyle \sum_{u \in V} s[u] \h{2} w[a-i, b-j] \nonumber\\
% \text{\ie } f(s) & = s \ast w \nonumber
% \end{align}
\end{proof}
