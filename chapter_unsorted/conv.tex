...
Convolution operators on $\cs(V)$ are equivariant to $\varphi(\Gamma)$ \ie
\begin{gather*}
...
The group convolution between two signals $s_1$ and $s_2 \in \cs(V)$ is defined as:
\begin{gather*}
\forall u \in V, (s_1 \ast s_2) [u] = \displaystyle \sum_{v \in V} s_1[v] \h{2} s_2[\varphi(g_v^{-1}g_u)] = \displaystyle \sum_{v \in V} s_1[v] \h{2} s_2[g_v^{-1}(u)]\\
...
Convolution operators on $\cs(V)$ are equivariant to $\Gamma$ \ie
\begin{gather*}
