...
In fact, both group convolutions are the same as the latter one borrows the algebraic structure of the first one. However, the second one is only equivariant to $\varphi(\Gamma)$, which is not exactly $\Gamma$, despite the fact that $\varphi$ is an isomorphism. To alleviate this issue, we need to take into account the fact that the group $\Gamma$ contains bijective transformations of $V$.
...
\begin{definition}\textbf{$\varphi$-coherence}\\
...
Let a subgroup $\Gamma \subset \Phi^*(V)$ that is \emph{$\varphi$-coherent}.
The group convolution between two signals $s_1$ and $s_2 \in \cs(V)$ is defined as:
\begin{align*}
\forall u \in V, (s_1 \ast s_2) [u] & = \displaystyle \sum_{v \in V} s_1[v] \h{2} s_2[\varphi(g_v^{-1}g_u)]\\
...
\begin{proof}
In the following equations, \eqref{eq:left} is obtained because left multiplication in a group is an isomorphism, and \eqref{eq:pty} is obtained because of \eqref{eq:P}.
\begin{align}
...
We now consider the class of Cayley (di)graphs because the $\varphi$-coherence property \eqref{eq:P} naturally holds on them.
...
\begin{gather*}
a \sim c \Leftrightarrow \exists b \in \varphi(\cu) \subset V, g_ag_b = g_c
\end{gather*}
\end{definition}
...
\begin{proof}
Let $\cu \subset \Gamma$ the generating set of the Cayley graph. \todo{}
\end{proof}
