...
\left\{
  \begin{array}{l}
    ...
    t_3[i_1, \ldots, i_n] =
    %\displaystyle \sum_{k_1, \ldots, k_n}
    \displaystyle \sum_{k_1=1}^{n_1^{(3)}} \cdots \sum_{k_n=1}^{n_n^{(3)}}
    t_1[i_1 + k_1 - 1, \ldots, i_n + k_n - 1] \hspace{2pt} t_2[k_1, \ldots, k_n] \\
  \end{array}
\right.
\end{gather*}
\label{convdef}
...
An \emph{activation function} $h$ defined from a tensor space to itself is defined as an univariate function applied dimension-wise and we use the functional notation $h(v)[i_1, i_2, \ldots, i_r] = h(v[i_1, i_2, \ldots, i_r])$.
...
% \begin{definition}\textbf{Connectivity matrix}\\
% Let $g$ a linear function. Without loss of generality subject to a flattening, let's suppose $I_g$ and $O_g$ are vector spaces. Then there exists a \emph{connectivity matrix}~$W_g$, and a multiset of parameters $\Theta_g$, such that:
% \begin{gather*}
% \left\{
% \begin{array}{l}
%   \forall \ovec{x} \in I_g, g(\ovec{x}) = W_g\ovec{x}\\
...
% \begin{definition}\textbf{Weights}\\
% The \emph{weights} of the $k$-th layer of a neural network, denoted $\Theta_k$, are defined as the multiset of parameters of its linear part.
...
\end{proof}
This allows to compute the gradients with a complexity that is linear with the number of weights (only one computation of the $\{x_k\}$), instead of being quadratic if it were done with the difference quotient expression of the derivatives (one more computation of the $\{x_k\}$ per weight).
\end{remark}
...
% not discussing geometric shape here (?)
% Maybe prove it as a proposition
\begin{remark}\textbf{Connectivity matrix of a convolution}\\
Note that a convolutional layer $(g,h)$ is equivalently defined as $W_g$ being a $n_{n+1}^{(I_g)} \times n_{n+1}^{(O_g)}$ block matrix such that its blocks are Toeplitz matrices.
\end{remark}

\begin{proof}
Let's recall from definition \ref{convdef} that
\begin{align*}
  y_q &= x_p \ast_n w_{p,q}[i_1, \ldots, i_n] \\
 &=
    \displaystyle \sum_{k_1=1}^{n_1^{(x_p)} - n_1^{(w_{p,q})} +1} \cdots \sum_{k_n=1}^{n_n^{(x_p)} - n_n^{(w_{p,q})} +1}
    x_p[i_1 + k_1 - 1, \ldots, i_n + k_n - 1] \hspace{2pt}
    w_{p,q}[k_1, \ldots, k_n]\\
 &= \displaystyle \sum_{j_1=i_1}^{i_1 + n_1^{(x_p)} - n_1^{(w_{p,q})}} \cdots \sum_{j_n=i_n}^{i_n + n_n^{(x_p)} - n_n^{(w_{p,q})}}
    x_p[j_1, \ldots, j_n] \hspace{2pt}
    w_{p,q}[j_1 - i_1 + 1, \ldots, j_n - i_n + 1]\\
 & \text{where } \forall p, j_p = i_p + k_p - 1
\end{align*}
Let a reshaping $w_{p,q} \mapsto \widetilde{w}_{p,q}$ such that its ranks are de-flattened using the inverses of the bijections $\{g_t: (i_t, j_t) \mapsto j_t - i_t + 1\}$. Then, subject to an index permutation and using notations as in (\ref{indices}), we can rewrite:
\begin{align*}
y_q \hspace{0pt}_{i_1^{\phantom{(}} \cdots i_n^{\phantom{(}}} = x_p \hspace{0pt}_{j_1^{\phantom{(}} \cdots j_n^{\phantom{(}}} \widetilde{w}_{p,q} \hspace{0pt}^{j_1^{\phantom{(}} \cdots j_n^{\phantom{(}}} \hspace{0pt}_{i_1^{\phantom{(}} \cdots i_n^{\phantom{(}}}
\end{align*}
where we recognize that the $n$-d convolution is expressed as a tensor contraction. Using the matrix product equivalence from remark~\ref{rq:matprodeq}, we can reshape $\widetilde{w}_{p,q}$ to a matrix $W_{p,q}$ and obtain the block $(p,q)$ of $W_g$.

The Toeplitz property holds for $\widetilde{w}_{p,q}$ for couples of covariant and contravariant indices $(i_t,j_t)$ because incrementing them both keeps the weight index $k_t$ constant. Hence, $W_{p,q}$ is a Toeplitz matrix. The proof of the converse also holds because reshapings are isomorphisms.
\end{proof}
