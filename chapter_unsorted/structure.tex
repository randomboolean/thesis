...
\begin{align*}
y_{j_1 \cdots j_n} = x_{i_1 \cdots i_n} \widetilde{w} \hspace{1pt}^{i_1 \cdots i_n} \hspace{0pt}_{j_1 \cdots j_n}
\end{align*}
...
\end{align*}
where we recognize that the $n$-d convolution can be expressed as a tensor contraction. Using the matrix product equivalence from remark~\ref{rq:matprodeq}, we can reshape $\widetilde{w}_{p,q}$ to a matrix $W_{p,q}$ and obtain the block $(p,q)$ of $W_g$. We denote by $g_i$ and $g_j$ the bijections in the index spaces related to this reshaping.

The Toeplitz property holds for $\widetilde{w}_{p,q}$ for couples of covariant and contravariant indices $(i_t,j_t)$ because incrementing them both keeps the weight index $k_t$ constant. If we.Hence, $W_{p,q}$ is a Toeplitz matrix. The converse is also proven because reshapings are isomorphisms.
\end{proof}
