...
A \emph{slice} operation $s$, along the last ranks $\{r_1, r_2, \ldots, r_s\}$, is a function that maps a tensor space $\tspace = \displaystyle \prod_{k=1}^r \vecspace_k$ to another tensor space $\tspace' = \displaystyle \prod_{k=1}^{r-s} \vecspace_k$, called a \emph{sliced subspace}, such that:
\begin{align*}
s(t)[i'_1, i'_2, \ldots, i'_{r-s}] &= t[i'_1, i'_2, \ldots, i'_{r-s}, i_{r_1}, i_{r_2}, \ldots, i_{r_s}] \\
\text{ i.e. } \quad s(t) :&= t[:,:, \ldots, :, i'_{r-s}, i_{r_1}, i_{r_2}, \ldots, i_{r_s}]
\end{align*}
We denote $t_{i_{r_1} i_{r_2} \cdots i_{r_s}}$ the slice of $t$ along the $s$ last ranks. 
