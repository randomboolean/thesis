...
In this section, we recall of rigorously redefine notions related to tensors, neural networks and graphs. Vector spaces considered below are assumed to be finite-dimensional and over the field of real numbers $\real$.

\subsection{Tensors}

\todo{rewrite and check}
Intuitively, tensors in deep learning are a generalization of vectors and matrices, as if vectors were tensors of rank $1$ and matrices were tensors of rank $2$. However, a tensor in mathematics is a multilinear object that is obtained from vectors of same dimensionality and from their duals. This does not generalize the notion of matrices as we need and thus is not directly usable in the context of deep learning. On the other hand, in tensor manipulation languages, tensors are basically defined as multidimensional array equipped with math operators and many others.

In this section, our aim is to provide a definition that is close to multidimensional arrays but that is more rigourous.
...
That is, a tensor space is a vector space. However, from now on for naming conveniency, we will abusively use the term vector space only to refer to tensor space of rank $1$, and matrices spaces to refer to tensor spaces of rank $2$.
...
An \emph{entry} of $t$ is a scalar denoted $t[i_1, i_2, \ldots, i_r]$.

A \emph{subtensor} $t'$ is a tensor of same rank composed of entries of $t$ that are contiguous in the indexing. We denote $t' = t[l_1{:}u_1, l_2{:}u_2, \ldots, l_r{:}u_r]$, where $\{l_p\}$ and $\{u_p\}$ are the lower and upper bounds of the indices used by the entries that compose~$t'$.
\end{definition}
