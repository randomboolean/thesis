...
\begin{enumerate}[nolistsep,noitemsep,label=(\roman*)]
  ...
  \item $f$ is a group convolution commutative operator $\Leftrightarrow$ $f$ is equivariant to multiplications $\Leftrightarrow$ $\Gamma$ is abelian \label{enum:ppp}
\end{enumerate}
\label{th:cgco}
\end{theorem}
\begin{proof}
...
The two expressions differ on the domain upon which the summation is done. The expression \eqref{eq:vdom} put the emphasis on each vertex and its action, whereas the expression \eqref{eq:premix} put the emphasis on each transformation of $\Gamma$.
...
\end{corollary}
\begin{proof}
As per bullet \ref{enum:pp} of \thref{th:cgco}.
\end{proof}
