...
Similarly with the construction under group action from \secref{sec:groupaction}, we are also seeking a groupoid partial auto-action. This time, left multiplicative action isn't as useful because the restriction $\alpha(p) = \beta(q)$ renders its usage too narrow. Instead, we use $\gamma: \cu \ltimes G \times V \rightarrow V$ and define an auto-action denoted by the same symbol $\gamma: \cu \ltimes G \times \cu \ltimes G \rightarrow \cu \ltimes G$.

\begin{definition}\textbf{Path auto-action}\\
The \emph{path auto-action} $\gamma$ is a groupoid partial action of a path groupoid $\cu \ltimes G$ on itself. Let 
$p = (g_n,v_n) \cdots (g_1,v_1)$ and $q = (h_m,u_m) \cdots (h_1,u_1)$, we have:
 \begin{align*}
 & q \in \cd_{\gamma_p} \Leftrightarrow
  \begin{cases}
    \beta_1(q) := \beta(q) \in \cd_{g_1}\\
    \forall r \in \seq{n-1}, \beta_{r+1}(q) \in \cd_{g_{r+1}}\\
    \text{where } \beta_{r+1}(q) = g_r \cdots g_2g_1(\beta(q)) 
  \end{cases}\\
  &\gamma_p(q) = p^qq \text{ where }\\
  &p^q = (g_n, \beta_n(q)) \cdots (g_2,\beta_2(q), (g_1,\beta(q))
 \end{align*}
\end{definition}
