...
\begin{remark}
Note that this definition is just a step in our construction since we will see that it is not enough to obtain the complete counterpart of \propref{prop:equi}.
\end{remark}
...
For convolution II, we only obtain a weak version of \propref{prop:equi}. Given a source set $A$ and destination set $B$, we denote, if defined, $\bij(A,B)$ the sets of their bijections $A \to B$ and $\iso(A,B)$ the sets of their isomorphims $A \to B$.
